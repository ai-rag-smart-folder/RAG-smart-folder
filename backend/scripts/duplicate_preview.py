#!/usr/bin/env python3
"""
Local Duplicate Image Preview Tool
Displays duplicate images locally without uploading to any server.
"""
...
class DuplicatePreviewTool:
    """Tool for previewing duplicate images locally."""
    
    def __init__(self, db_path: str = "data/dev.db", api_url: str = "http://127.0.0.1:8003"):
        self.db_path = db_path
        self.api_url = api_url
        self.temp_dir = None
        
    def get_duplicates_from_api(self, mode: str = "exact", confidence_threshold: float = 80.0) -> Dict[str, Any]:
        """Fetch duplicate detection results from the API."""
        try:
            url = f"{self.api_url}/duplicates"
            params = {
                "mode": mode,
                "confidence_threshold": confidence_threshold
            }
            
            print(f"Fetching duplicates from {url}...")
            ...
            print("Make sure the backend server is running with: uvicorn app.main:app --host 127.0.0.1 --port 8003")
            return None
            ...
            for row in cursor.fetchall():
                ...
                for file_row in cursor.fetchall():
                    ...
                    files.append({
                        ...
                        "width": width,
                        "height": height,
                        "is_original": file_size == min(f[3] for f in cursor.execute(
                            f"SELECT file_size FROM files WHERE id IN ({placeholders})", file_ids
                        ).fetchall())
                    })
...
def main():
    parser = argparse.ArgumentParser(description="Local Duplicate Image Preview Tool")
    parser.add_argument("--db", default="data/dev.db", help="Database path")
    parser.add_argument("--api-url", default="http://127.0.0.1:8003", help="API URL")
    parser.add_argument("--mode", choices=["exact", "similar", "comprehensive"], default="exact", 
                       help="Detection mode")
    parser.add_argument("--confidence", type=float, default=80.0, help="Confidence threshold")
    ...
    tool = DuplicatePreviewTool(db_path=args.db, api_url=args.api_url)
    
    try:
        print("🔍 Fetching duplicate detection results...")
        
        # Try API first, fallback to direct DB access
        duplicates_data = tool.get_duplicates_from_api(args.mode, args.confidence)
        if not duplicates_data:
            print("⚠️  API not available, trying direct database access...")
